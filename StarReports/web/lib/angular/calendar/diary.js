...
// TODO: rename all header-related things to "toolbar"
	...
	function Header(calendar, options) {
      ...
      var title_for_today = text == moment().format('dddd Do MMM YYYY'),
          title = text;
		...
		getCell: function (x, y) {
			...
			if (this.inBounds(x, y)) {
				...
				if (r >= 0 && c >= 0) {
					...
					if (this.grid.view.name == "agendaResource" && this.grid instanceof AllDayGrid) {
						cell.date = this.grid.getCellDate({row: 0, col: 0});
						cell.allDayVal = r ? 'pm' : 'am';
					} else if (this.grid.view.name == "agendaWeek" && this.grid instanceof AllDayGrid) {
						cell.date = this.grid.getCellDate({row: 0, col: c});
						cell.allDayVal = r ? 'pm' : 'am';
					}
				}
			}
...
// (such as highlight rows, day rows, helper rows, etc).
	...
	RowRenderer.prototype = {
		...
		rowHtml: function (rowType, row) {
			...
			var rowVal = (view.name == "agendaResource" || view.name == "agendaWeek") ? 0 : row || 0;

			for (col = 0; col < view.colCnt; col++) {
				var colVal = col;
				if (view.name == "agendaResource") {
					colVal = 0;
				}
				date = view.cellToDate(rowVal, colVal);
	...
	$.extend(Grid.prototype, {
        ...
        getCSEventDesign: function(event, timeText){
            ...
            timeText = (event.allDay) ? event.allDay : this.view.getEventTimeText(event, 'hh:mm');
            ...
            if(!event.allDay && !event.isSpecialEvent) {
                resizer = '<span class="cs-event-resizer fc-resizer"></span>';
            }
            ...
            if(event.type == 'estimate' || event.type == 'job') {
                ...
                var addressDetails = '<p class="address"></span>' + workAddressImage + event.otherDetails.addressline + ',<strong>' + event.otherDetails.postcode + '</strong></p><span class="contact-number">'+phoneSymDesign+
                    '<span>' + event.otherDetails.contactNumber +'</span></span>';

            }

            //TODO:Need to move this to html
            if(event.type == 'normal') {
                customHtml = '<div class="cs-event '+additionalClasses+'" '+popUpAttributes+'>' +specialEventDesign+
                ...
                customHtml = '<div class="cs-event '+additionalClasses+'" '+popUpAttributes+' '+doubleClick+'>' +specialEventDesign+
                ...
                '<span class="cs-event-header-time">' + timeText + '</span>' +
                '<span class="cs-event-header-text">' + event.otherDetails.template + '</span>' +
                '<span class="cs-event-header-status">' +
                '<span class="diary-event-status ' + event.status.class + '"></span>' +
                '</span>' +
                '</div>' +
                ...
                '<span class="property-details">' +workAddress+customerName+addressDetails+
                '</span>' +
                '</div>' +
                '</div>' + resizer +
                '</div>' +
                ...
                '<div class="cs-event-inner">'+
                    '<div class="cs-event-header">'+
                        ...
                        '</div>'+
                        '<div class="cs-event-header-lower">'+
                            '<span class="cs-event-header-user">'+img+'<span>' + event.userName + '</span>'+
                                '</span>'+
                            '</div>'+
                            '<hr>'+
                            '</div>'+
			...
			var dragListener = new DragListener(this.coordMap, {
				...
				cellOver: function (cell, date) {
					if (dragListener.origDate) { // click needs to have started on a cell
                        var newEndVal = false;
                        if(cell.col != dragListener.origCell.col || cell.row != dragListener.origCell.row) { //Temp fix to avoid multiple day selection
							if (view.name == "twoWeek" || view.name == "month") {
								dragListener.origCell.row = cell.row;
								dragListener.origDate = cell.date;
                                newEndVal = cell.date.clone().add(1, 'day').startOf('day');
                            }
							if(view.name == 'agendaWeek' && cell.col != dragListener.origCell.col) {
								dragListener.origDate = cell.date;
							}
                            dragListener.origCell.col = cell.col;
                        }
						dayEl = _this.getCellDayEl(cell);

						dates = [ date, dragListener.origDate ].sort(dateCompare);
						start = dates[0];
						end = (newEndVal) ? newEndVal : dates[1].clone().add(_this.cellDuration);

						if (isSelectable) {
							if (view.name == "agendaResource" || view.name == "agendaWeek") {
								if (dragListener.cell.allDayVal && (dragListener.cell.allDayVal != dragListener.origCell.allDayVal)) {
									cell.allDayVal = 'fullday';
								}
							}
							_this.renderSelection(start, end, cell);
						}
					}
				},
					...
					if (dates) { // started and ended on a cell?
						...
						if (view.name == "agendaResource" || view.name == "agendaWeek") {
							if (view.name != "agendaWeek") {
								resourceId = view.calendar.resources[dragListener.cell.col].id;
							}
							if (dragListener.cell.allDayVal && (dragListener.cell.allDayVal != dragListener.origCell.allDayVal)) {
								allDayVal = 'fullday';
							} else {
								allDayVal = dragListener.cell.allDayVal;
							}
                            if(dragListener.cell.allDayVal == 'am') {
                                end = start.clone().add(12, 'h');
                            } else if(dragListener.cell.allDayVal == 'pm') {
                                start = start.clone().add(12, 'h');
                                end = start.clone().add(12, 'h');
                            }
						}
						if (dates[0].isSame(dates[1])) {
							view.trigger('dayClick', dayEl[0], start, ev, resourceId, allDayVal);
						}
						if (isSelectable) {
							// the selection will already have been rendered. just report it
							view.reportSelection(start, end, ev, resourceId, allDayVal);
						}
					}
				}
			});
		...
		renderRangeHelper: function (start, end, sourceSeg, cell) {
			var view = this.view;
			var fakeEvent;
		...
		renderSelection: function (start, end, cell) {
			this.renderHighlight(start, end, cell);
		},
      ...
      // Callum's custom start
      var header_for_today = moment(moment().format('ll')).isSame(date.format('ll'));

      if(header_for_today) {
        view.widgetHeaderClass += ' header-for-today';
      }else {
        view.widgetHeaderClass = view.widgetHeaderClass.replace(' header-for-today', '');
      }
      // Callum's custom end
	...
	$.extend(Grid.prototype, {
		...
		eventToSegs: function (event, intervalStart, intervalEnd) {
			...
			if (intervalStart && intervalEnd) {
				...
				var cellObj = false;
				if (this.view.name == "agendaResource") {
					var colVal = this.getCol(event.resourceId);
					cellObj = {col: colVal, allDayVal: event.allDay, singleResp: true};
				} else if (this.view.name == "agendaWeek") {
					cellObj = {allDayVal: event.allDay, singleResp: true};
				}
				segs = this.rangeToSegs(eventStart, eventEnd, cellObj); // defined by the subclass
			}
			...
			for (i = 0; i < segs.length; i++) {
				...
				seg.event = event;
				if (this.view.name == "agendaResource") {
					seg.col = (event.col >= 0) ? event.col : this.getCol(event.resourceId);
				}
				if (this.view.name == "agendaWeek" && typeof seg.col == 'undefined') {
					seg.col = seg.leftCol;
				}
		...
		getCol: function (resourceId) {
			var resources = this.view.calendar.resources;
		...
		segDragMousedown: function (seg, ev) {
			...
			var el = seg.el;
			var event = seg.event;
			var newStart, newEnd, newResourceId, allDayValue;
			...
			var dragListener = new DragListener(view.coordMap, {
				...
				cellOver: function (cell, date) {
					...
					if(view.name == "agendaResource") {
						newResourceId = view.calendar.resources[cell.col].id;
					}
					...
					if(cell.allDayVal) {
						...
						allDayValue = cell.allDayVal;
					} else {
                        if(view.name == "twoWeek" || view.name == "month") {
                            allDayValue = event.allDay;
                        } else {
                            allDayValue = false;
                        }
					}

					if (view.renderDrag(newStart, newEnd, seg, cell)) { // have the view render a visual indication
				...
				dragStop: function (ev) {
                    ...
                    if(!allDayValue && event.allDay) {
                        //event._allDay = allDayValue;
                    } else {
                        event._allDay = false;
                    }

					if(allDayValue != event.allDay) {
						event.allDay = allDayValue;
						hasChanged = true;
					}
                    ...
                    if(event.isSpecialEvent && event.allDay) {
                        hasChanged = false;
                    }
		...
		segResizeMousedown: function (seg, ev) {
			...
			var newEnd = null;
			var dragListener;
            var oldEndToCheck = start.clone().add(1, 'day').startOf('day');
			...
			dragListener = new DragListener(this.coordMap, {
				...
				cellOver: function (cell, date) {
					...
					if (newEnd.isSame(end)) {
						newEnd = null;
						destroy();
					} else if( newEnd > oldEndToCheck){
                        newEnd = null;
                    } else {
						_this.renderResize(start, newEnd, seg, dragListener.cell);
...
// A cmp function for determining which segments should take visual priority
	...
	$.extend(DayGrid.prototype, {
		...
		dayRowHtml: function (row, isRigid) {
			var view = this.view;
			//santha:row = (view.name == "agendaResource" || view.name == "agendaWeek") ? 0 : row ||0;
		...
		// Converts a range with an inclusive `start` and an exclusive `end` into an array of segment objects
		rangeToSegs: function (start, end, cell) {
			return this.view.rangeToSegments(start, end, cell); // leverages the View's cell system
		},
		...
		// `end` can be null, as well as `seg`. See View's documentation on renderDrag for more info.
		// A returned value of `true` signals that a mock "helper" event has been rendered.
		renderDrag: function (start, end, seg, cell) {
			var opacity;
			...
			if (seg && !seg.el.closest(this.el).length) {
				...
				this.renderRangeHelper(start, end, seg, cell);
		...
		// Renders a visual indication of an event being resized
		renderResize: function (start, end, seg, cell) {
		...
		renderHighlight: function (start, end, cell) {
			var segs = this.rangeToSegs(start, end, cell);
			var highlightNodes = [];
			var i, seg;
			var el;
	...
	$.extend(DayGrid.prototype, {
		...
		renderSegHtml: function (seg, disableResizing) {
            ...
            return this.getCSEventDesign(event, view.getEventTimeText(event));
		},
	...
	function AllDayGrid(view) {
		Grid.call(this, view); // call the super-constructor
	}


	AllDayGrid.prototype = createObject(Grid.prototype); // declare the super-class
	$.extend(AllDayGrid.prototype, {
		...
		highlightSkeletonHtml: function (startCol, endCol) {
			...
			return '' +
				'<div class="fc-highlight-skeleton all-day-highlight-skeleton">' +
				'<table>' +
				'<tr>' +
				cellHtml +
				'</tr>' +
	...
	$.extend(AllDayGrid.prototype, {
		...
		renderEvents: function (events) {
            ...
            var fcScroller = $('.fc-scroller')[0];
            var fcTimeGrid = $('.fc-time-grid')[0];
            var marginRight = 0;
            if(fcScroller && fcTimeGrid) {
                marginRight = fcScroller.offsetWidth - fcTimeGrid.offsetWidth - 1;
            }
			var marginLeft = this.el.find('td.fc-widget-content')[0].clientWidth;
			this.eventSkeletonEl = $('<div class="fc-content-skeleton"/>').css({marginRight: marginRight+'px', top: 0, left: 0, position: "absolute"}).append(res.tableEl);
		...
		renderEventTable: function (events) {
			...
			var allDayDetails = this.setLevelDetails(events);
			segs = this.renderSegs(segs); // returns only the visible segs
			var newSeg = [];
			for (var segI = 0; segI < segs.length; segI++) {
				newSeg.push(this.setLevelValues(segs[segI], allDayDetails));
			}
			segCols = this.groupSegCols(segs); // group into sub-arrays, and assigns 'col' to each seg


			//this.computeSegVerticals(segs); // compute and assign top/bottom
			var slotHeight = this.el.find('table')[0].clientHeight;

			for (col = 0; col < segCols.length; col++) { // iterate each column grouping
				colSegs = segCols[col];
				//placeSlotSegs(colSegs); // compute horizontal coordinates, z-index's, and reorder the array

				containerEl = $('<div class="fc-event-container all-day-grid-container"/>');

				// assign positioning CSS and insert into container
				for (i = 0; i < colSegs.length; i++) {
					seg = colSegs[i];
					var levelCnt = this.getLevelCount(seg.event, allDayDetails);
					seg.el.css(this.generateSegPositionCss(seg, slotHeight, levelCnt));
		...
		generateSegPositionCss: function (seg, slotHeight, levelCnt) {
			var fullDayCount = seg.fullDayCount;
			var allDay = seg.event.allDay;
			var level = seg.level + fullDayCount;
            var maxLevelCount = levelCnt;
            var timeGridContainer = $('.fc-time-grid-container')[0];
            var dayGridContainer = $('.fc-day-grid')[0];
            var bottom = timeGridContainer.offsetTop - dayGridContainer.offsetTop - 2;

            if(allDay == 'am') {
                bottom -= slotHeight;
            }

			if (seg.fullDayCount == 0) {
				levelCnt = seg.levelCount;
			} else {
				levelCnt = (allDay == 'fullday') ? fullDayCount + levelCnt : seg.levelCount + fullDayCount;
			}

            var normalCount = levelCnt - fullDayCount;
            var availableWidth = 100;
            var leftValToAdd = 0;
            var countToCheck = normalCount;
            var zIndex = 1;
            level = seg.level;
			if (allDay == 'fullday') {
				slotHeight = slotHeight * 2;
                zIndex = level = seg.fullDayLevel;
                countToCheck = levelCnt;
			} else {
                zIndex = seg.level + fullDayCount;
                if(fullDayCount > 0) {
                    var fullDayWidth = Math.round(availableWidth / (maxLevelCount + fullDayCount));
                    var widthTakenToFullDay = fullDayWidth * fullDayCount;
                    leftValToAdd = widthTakenToFullDay - Math.round(fullDayWidth / 3);
                    availableWidth = 100 - leftValToAdd;
                }
			}

			var top = seg.row * slotHeight - 1;
			var width = Math.round(availableWidth / countToCheck);
			var props = {top: top};
			var left = leftValToAdd + level * width;
            var rightValToAdd = (level != countToCheck - 1) ? width / 2 : 0;
			var right = (countToCheck - level - 1) * width - rightValToAdd;

			props.zIndex = zIndex + 1;
			props.left = left + '%';
			props.right = Math.round(right) + '%';
			props.bottom = -bottom;
		...
		renderSegHtml: function (seg, disableResizing) {
            ...
            var customHtml = '';
            if(event.id > 0) {
                customHtml = this.getCSEventDesign(event, event.allDay);
            }
            return customHtml;
		},
...
// A cmp function for determining the leftmost event
	...
	$.extend(TimeGrid.prototype, {
		...
		rangeToSegs: function (rangeStart, rangeEnd) {
			...
			var colCntValue = 1;
			if (this.view.name != "agendaResource") {
				colCntValue = view.colCnt;
			}

			for (col = 0; col < colCntValue; col++) {
		...
		renderSelection: function (start, end, cell) {
			if (this.view.opt('selectHelper')) { // this setting signals that a mock helper event should be rendered
				this.renderRangeHelper(start, end, false, cell);
			}
			else {
				this.renderHighlight(start, end, cell);
			}
		...
		highlightSkeletonHtml: function (start, end, cell) {
			var view = this.view;
			var segs = this.rangeToSegs(start, end, cell);
			var cellHtml = '';
			var col = 0;
	...
	$.extend(TimeGrid.prototype, {
		...
		renderEvents: function (events) {
            if(this.timeSlotHeight == 0){
                this.timeSlotHeight = $('td.fc-axis.fc-time.fc-widget-content')[0].offsetHeight;
            }
			var res = this.renderEventTable(events);
		...
		renderEventTable: function (events) {
			...
			for (col = 0; col < segCols.length; col++) { // iterate each column grouping
				...
				for (i = 0; i < colSegs.length; i++) {
                    ...
                    if(seg.event.isSpecialEvent) {
                        seg = this.handleSpecialEvent(seg);
                        seg.el.css(this.generateSegPositionCss(seg));
                    }
		...
		updateSegVerticals: function () {
			...
			if (segs) {
				...
				for (i = 0; i < segs.length; i++) {
                    var segment = segs[i];
                    if(segment.event.isSpecialEvent) {
                        segment = this.handleSpecialEvent(segment);
                    }
                    segment.el.css(
						this.generateSegVerticalCss(segment)
					);
				}
			}
		},
        ...
        //This is to handle the special event design
        handleSpecialEvent: function(seg){
            var timeDiff = seg.event.end.diff(seg.event.start)/1000/60;
            var numOfSlotsForEvent = timeDiff / 30;
            var numOfSlotsForSpan = parseInt(seg.event.actualTime) / 30;
			if(numOfSlotsForSpan > numOfSlotsForEvent) {
				numOfSlotsForSpan = numOfSlotsForEvent;
			}
            var spanSlot = numOfSlotsForEvent - numOfSlotsForSpan;
            var spanHeight = spanSlot * this.timeSlotHeight / 2;
            var spanHeightToSet = spanHeight - 2;
            var spanTopBottom = -spanHeight;
            var specialEventSpanEls = seg.el.find('span.cs-special-event-spacer');
            $(specialEventSpanEls[0]).css({height: spanHeightToSet + 'px', top: spanTopBottom + 'px'});
            $(specialEventSpanEls[1]).css({height: spanHeightToSet + 'px', bottom: spanTopBottom + 'px'});
            seg.bottom = seg.bottom + spanTopBottom;
            seg.top = seg.top - spanTopBottom;
            return seg;
        },
		...
		renderSegHtml: function (seg, disableResizing) {
			...
			var customHtml = '';
			if(event.id > 0) {
				customHtml = this.getCSEventDesign(event, timeText);
			} else {
		...
		// CS-NOTE: I (Callum) have deducted 1 from the bottom pos of the seg so that alignment of the
		// bottom of event is in line with with time in the axis
		generateSegVerticalCss: function (seg) {
			return {
				top: seg.top,
				bottom: -seg.bottom-1 // flipped because needs to be space beyond bottom edge of event container
			};
		},
...
// Newer methods should be written as prototype methods, not in the monster `View` function at the bottom.
	...
	View.prototype = {
			...
			if (this.opt('droppable')) { // only listen if this setting is on
				...
				dragListener = new DragListener(this.coordMap, {
					cellOver: function (cell, date) {
                        cellObj = cell;
						dropDate = date;
                        _this.renderDrag(date, null, {}, cell);
					},
					cellOut: function () {
						dropDate = null;
						_this.destroyDrag();
				...
				$(document).one('dragstop', function (ev, ui) {
					...
					if (dropDate) {
                        var resourceId = false;
                        if(_this.name == 'agendaResource') {
                            resourceId = _this.calendar.resources[cellObj.col].id;
                        }

                        var allDayValue = (!cellObj.allDayVal) ? false : cellObj.allDayVal;

						_this.trigger('drop', ev.target, dropDate, ev, ui, resourceId, allDayValue);
					}
				});
		...
		select: function (start, end, ev, resourceId) {
			this.unselect(ev);
			this.renderSelection(start, end);
			this.reportSelection(start, end, ev, resourceId);
		},
		...
		reportSelection: function (start, end, ev, resourceId, allDay) {
			this.isSelected = true;
			this.trigger('select', null, start, end, ev, resourceId, allDay);
		},
		...
		documentMousedown: function (ev) {
			...
			if (this.isSelected && this.opt('unselectAuto') && isPrimaryMouseButton(ev)) {
                ...
                var unselectOptions = this.opt('unselectCancel');
                var doUnselect = true;
                if(typeof unselectOptions == 'object') {
                    for(var index = 0; index < unselectOptions.length; index++) {
                        var ignore = unselectOptions[index];
                        if ($(ev.target).closest(ignore).length) {
                            doUnselect = false;
                            break;
                        }
                    }
                } else {
                    if (!unselectOptions || !$(ev.target).closest(unselectOptions).length) {
                        doUnselect = false;
                    }
                }
...
// constructor. Going forward, methods should be part of the prototype.
	function View(calendar) {
		...
		// - isEnd
		//
		function rangeToSegments(start, end, cell) {

			var rowCnt = t.rowCnt;
			var colCnt = t.colCnt;
			if (t.name == "agendaResource") {
				rowCnt = 1;
				colCnt = 1;
			...
			for (var row = 0; row < rowCnt; row++) {
				...
				if (segmentCellOffsetFirst <= segmentCellOffsetLast) {
					...
					if (cell && cell.allDayVal) {
						if (typeof cell.col != "undefined") {
							cols[0] = cell.col;
							cols[1] = cell.col;
						}
						if (cell.allDayVal == 'pm') {
							row = 1;
						} else if (cell.allDayVal == 'am') {
							row = 0;
						} else if (cell.allDayVal == 'fullday') {
							row = 0;
							if (!cell.singleResp) {
								segments.push({
									row: 1,
...
// It is responsible for managing width/height.
	...
	fcViews.twoWeek = TwoWeeklyView; // register the view

	function TwoWeeklyView(calendar) {
		BasicView.call(this, calendar); // call the super-constructor
	}

	TwoWeeklyView.prototype = createObject(BasicView.prototype); // define the super-class
	$.extend(TwoWeeklyView.prototype, {

		name: 'twoWeek',


		incrementDate: function (date, delta) {
			return date.clone().stripTime().add(2 * delta, 'weeks').startOf('week');
		},
		...
		render: function (date) {
			...
			this.title = this.calendar.formatRange(
				this.start,
				this.intervalEnd.clone().subtract(1), // make inclusive by subtracting 1 ms
...
// Responsible for managing width/height.
	...
	function AgendaView(calendar) {
		...
		if (this.opt('allDaySlot')) { // should we display the "all-day" area?
			this.dayGrid = new AllDayGrid(this); // the all-day subcomponent of this view
			this.dayGrid.setLevelValues = function (eventSegments, allDayLevelDetails) {
				var dateStr = eventSegments.event.start.format("YYYY-MM-DD");
				var levelObj = allDayLevelDetails[eventSegments.row][dateStr];
				var seg = eventSegments;
				var allDay = seg.event.allDay;
			...
			this.dayGrid.setLevelDetails = function (events) {
				var levelDetails = [];
					...
					for (var j = 0; j < events.length; j++) {
						...
						if (typeof levelDetails[row][dateString] != "undefined") {
							if (events[j].allDay == 'fullday' || (row == 0 && events[j].allDay == 'am') || (row == 1 && events[j].allDay == 'pm')) {
								var levelCount = (events[j].allDay != 'fullday') ? levelDetails[row][dateString].levelCount + 1 : levelDetails[row][dateString].levelCount;
							...
							if (events[j].allDay == 'fullday' || (row == 0 && events[j].allDay == 'am') || (row == 1 && events[j].allDay == 'pm')) {
								var fullDayCount = this.getFullDayCount(dateString, events);
								var levelCnt = 1;
			...
			this.dayGrid.getFullDayCount = function (dateString, events) {
				var count = 0;
			...
			this.dayGrid.getLevelCount = function (event, allDayLevelDetails) {
				var count = 0;
				var eventDate = event.start.format("YYYY-MM-DD");
				var rowCnt1 = (typeof allDayLevelDetails[0][eventDate] == "undefined") ? 0 : allDayLevelDetails[0][eventDate].levelCount;
				var rowCnt2 = (typeof allDayLevelDetails[1][eventDate] == "undefined") ? 0 : allDayLevelDetails[1][eventDate].levelCount;
				if (rowCnt1 > rowCnt2) {
					count = rowCnt1;
				} else {
					count = rowCnt2;
	...
	$.extend(AgendaView.prototype, {
		...
		headIntroHtml: function () {
			...
			if (this.opt('weekNumbers')) {
                ...
                var headerClasses = this.widgetHeaderClass.replace(' header-for-today', '');//To remove this class from fc axis td
				return '<th class="fc-axis ' + headerClasses + '" ' + this.axisStyleAttr() + '></th>';
			}
		},
		...
		// Queried by the DayGrid subcomponent when generating rows. Ordering depends on isRTL.
		dayIntroHtml: function () {
			var allDayText = arguments[0] ? 'pm' : 'am';
			return '' +
		...
		renderDrag: function (start, end, seg, cell) {
			if (start.hasTime()) {
				return this.timeGrid.renderDrag(start, end, seg, cell);
			}
			else if (this.dayGrid) {
				return this.dayGrid.renderDrag(start, end, seg, cell);
			}
		},
...
// TODO: a WeekView mixin for calculating dates and titles
	...
	function ResourceView(calendar) {
		...
		this.timeGrid = new TimeGrid(this);
		this.timeGrid.getCellDate = function (cell) {
			var view = this.view;
			var calendar = view.calendar;

			return calendar.rezoneDate( // since we are adding a time, it needs to be in the calendar's timezone
				view.cellToDate(0, 0) // View's coord system only accounts for start-of-day for column
					.time(this.minTime + this.snapDuration * cell.row)
			);
		}
		...
		if (this.opt('allDaySlot')) { // should we display the "all-day" area?
					...
					for (var resCnt = 0; resCnt < resources.length; resCnt++) {
						...
						for (var j = 0; j < events.length; j++) {
							...
							if (typeof levelDetails[i][resource.id][dateString] != "undefined") {
								...
								var fullDayCount = this.getFullDayCount(resource.id, dateString, events);
						...
						if (events[i].resourceId === resources[resCnt].id) {
							...
							for (var row = 0; row < 2; row++) {
								if (newEvent.allDay == 'fullday' || (row == 0 && newEvent.allDay == 'am') || (row == 1 && newEvent.allDay == 'pm')) {
									var levelCount = (newEvent.allDay != 'fullday') ? levelDetails[row][resourceId][dateString].levelCount + 1 : levelDetails[row][resourceId][dateString].levelCount;
			...
			this.dayGrid.getFullDayCount = function (resourceId, dateString, events) {
				var count = 0;
				for (var i = 0; i < events.length; i++) {
					var newEvent = events[i];
	...
	fcViews.agendaResource = agendaResourceView; // register the view

	function agendaResourceView(calendar) {
		ResourceView.call(this, calendar); // call the super-constructor
	}


	agendaResourceView.prototype = createObject(ResourceView.prototype); // define the super-class
	$.extend(agendaResourceView.prototype, {

		name: 'agendaResource',


		incrementDate: function (date, delta) {
			return date.clone().stripTime().add(delta, 'days');
		},
		...
		render: function (date) {
			...
			ResourceView.prototype.render.call(this, this.calendar.resources.length); // call the super-method
		}
	...
	$.extend(ListView.prototype, {
		...
		renderEvents: function renderListEvents(events) {
			...
			this.eventsOfThisDay = function (events, dateToCheck) {
				var retArr = new Array();
				for (i in events) {
					if (events[i].start.format('YYYY-MM-DD') == dateToCheck.format('YYYY-MM-DD')) {
			...
			this.getListDateString = function (event) {
				var ret = "";
			...
			var sortedEvents = newEvents.sort(this.sortEvent);
			var hasEvents = false;
			var total = sortedEvents.length;

			var x = '<section id="diary-list-view">'+
            '<table class="diary-list-view-header">'+
            '<thead>'+
                '<tr>'+
                    '<th>Event</th>'+
                    '<th>Date</th>'+
            ...
            '</thead>'+
            '</table>'+
            '<div class="diary-list-view-main-wrap">'+
                '<table class="diary-list-view-main">'+
                    '<tbody>';
			...
			if (total > 0) {
				...
				while (dateVar <= end) {
					var arr = this.eventsOfThisDay(sortedEvents, dateVar);
                    if (arr.length > 0) {
                        hasEvents = true;
                        for (i in arr) {
                            var event = arr[i];
                            var dateStr = moment(event.start).format('ddd D MMM');
                            var timeText = this.getListDateString(event);
                            var img = '<span class="ss-user"></span>';
                            var workAddressName = '';
                            var workAddressClass = '';
                            var statusToShow = (event.status.status != 'just_booked') ? event.status.status : '';
                            if(event.otherDetails.work_addrs_name) {
                                workAddressName = event.otherDetails.work_addrs_name;
                                workAddressClass = 'ss-buildings';
                            }
                            if(event.userProfilePicture) {
                                img = '<img src="'+event.userProfilePicture+'">';
                            }
                            if(event.type == 'normal') {
                                ...
                                '<div class="cs-event-header">'+
                                    ...
                                    '<div class="cs-event-header-lower">'+
                                        '<span class="cs-event-header-user">'+img+
                                            '<span>'+event.userName+'</span>'+
                                        '</span>'+
                                        '<span class="cs-event-estimate-or-job-status">'+
                                            '<span class="estimate-status '+ event.otherDetails.estimateStatus +'">&nbsp;</span> Estimate '+ event.otherDetails.estimateStatus +
                                        '</span>'+
                                    '</div>'+
                                '</div>'+
                                '</td>'+
                                ...
                                '<div class="property-item">'+
                                    '<span class="property-details">'+
                                        '<strong>'+ workAddressName +'</strong>'+
                                        '<strong>'+event.otherDetails.customerName+'</strong>'+
                                        '<p class="address">'+
                                            '<span tooltip="Work address" tooltip-placement="top" class="'+workAddressClass+' ng-scope"></span>' + event.otherDetails.addressline + ','+
                                            '<strong>' + event.otherDetails.postcode + '</strong>'+
                                        '</p>'+
                                    '</span>'+
                                '</div>'+
                                '</td>'+
                                ...
                                x += '<tr class="for-job is-work-address">'+
                                '<td>'+
                                '<div class="cs-event-header">'+
                                    '<div class="cs-event-header-upper">'+
                                        '<span class="cs-event-header-event-type job">j</span>'+
                                    ...
                                    '<div class="cs-event-header-lower">'+
                                        ...
                                        '</span>'+
                                        '<span class="cs-event-estimate-or-job-status">'+
                                            '<span class="estimate-status">&nbsp;</span>'+//Class and status will appear here
                                        '</span>'+
                                    '</div>'+
                                '</div>'+
                                '</td>'+
			...
			if (!hasEvents) {
				x += "<tr><td colspan='5'>There is no events</td></tr>";
			}
			x += '</tbody>'+
            '</table>'+
            '</div>'+
